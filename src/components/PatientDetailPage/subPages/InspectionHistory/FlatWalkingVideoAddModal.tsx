...
const FlatWalkingVideoAddModal = ({show, handleClose, isNew=false, cv}: { show: boolean, handleClose: () => void, isNew?: boolean, cv: any }) => {
    ...
    const addNewPodoscope = async () => {
        ...
        try {
            await axios.post(url, newPodoscope, config)
        ...
        <Modal show={show} onShow={renderSelected} onHide={handleClose} size='xl'>
                                ...
                                <div className={`${cx("cell")}`}>
                                    <InputGroup>
                                        ...
                                        <Form.Control
                                            ...
                                            rows={3}
                                            value={podoscopeNote}
                                            onChange={(e) => setPodoscopeNote(e.target.value)}
                                        >
                                        </Form.Control>
                                    </InputGroup>
                                </div>
            ...
            <Modal.Footer>
                <div className={cx("inline-btn")}>
                    <Button variant="primary" onClick={addNewPodoscope}>
                        {isNew ? "추가": "변경"}
                    </Button>
...
export default FlatWalkingVideoAddModal
